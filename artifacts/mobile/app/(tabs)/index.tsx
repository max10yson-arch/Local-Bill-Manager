  ...
  return (
    <View style={styles.field}>
      ...
      <TextInput
        ...
        style={[styles.input, multiline && styles.textarea, { backgroundColor: colors.card, borderColor: colors.border, color: colors.foreground }]}
      />
    </View>
  );
}
...
export default function BillScreen() {
  ...
  const { products, customers, draft, settings, totals, stats, updateDraft, addItem, updateQty, removeItem, saveBill, clearDraft, loadCustomerIntoDraft, isReady } = useBilling();
    ...
    <View style={[styles.screen, { backgroundColor: colors.background, paddingTop: Platform.OS === "web" ? 67 : 0 }]}>
      <ScrollView contentContainerStyle={[styles.scroll, { paddingBottom: insets.bottom + 110 }]} showsVerticalScrollIndicator={false}>
        <View style={[styles.hero, { backgroundColor: colors.primary }]}>
          <View>
            <Text style={styles.heroKicker}>Parinay Saree</Text>
            <Text style={styles.heroTitle}>Bill desk</Text>
            <Text style={styles.heroText}>{stats.customerCount} customers · {formatCurrency(stats.totalRevenue)} billed</Text>
          </View>
          <View style={[styles.invoicePill, { backgroundColor: "rgba(255,255,255,0.16)" }]}>
            <Text style={styles.invoicePillLabel}>Invoice</Text>
            <Text style={styles.invoicePillValue}>{draft.invoiceNum}</Text>
          </View>
        </View>
        ...
        <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <View style={styles.cardHeader}>
            ...
            <Pressable testID="pick-customer" onPress={() => setCustomerPickerOpen(true)} style={[styles.smallButton, { backgroundColor: colors.roseSoft }]}>
              <Feather name="users" size={15} color={colors.primary} />
              <Text style={[styles.smallButtonText, { color: colors.primary }]}>Pick saved</Text>
            </Pressable>
          </View>
          ...
          <View style={styles.twoCol}>
            <Field label="Phone" value={draft.customerPhone} keyboardType="phone-pad" placeholder="+91" onChangeText={(customerPhone) => updateDraft({ customerPhone })} />
            <Field label="City / PIN" value={draft.customerCity} placeholder="Indore — 452001" onChangeText={(customerCity) => updateDraft({ customerCity })} />
          </View>
        ...
        <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <View style={styles.cardHeader}>
            <Text style={[styles.cardTitle, { color: colors.foreground }]}>Products</Text>
            <Pressable testID="add-product" onPress={() => setProductPickerOpen(true)} style={[styles.primaryMini, { backgroundColor: colors.primary }]}>
              <Feather name="plus" size={16} color={colors.primaryForeground} />
              <Text style={[styles.primaryMiniText, { color: colors.primaryForeground }]}>Add</Text>
            </Pressable>
          </View>
          {draft.items.length === 0 ? (
            <View style={[styles.emptyBox, { backgroundColor: colors.goldSoft }]}>
              <Feather name="shopping-bag" size={24} color={colors.accent} />
              <Text style={[styles.emptyTitle, { color: colors.foreground }]}>No items yet</Text>
              <Text style={[styles.emptyText, { color: colors.mutedForeground }]}>Add catalog or custom products to build the bill.</Text>
            </View>
          ) : draft.items.map((item) => (
            <View key={item.uid} style={[styles.itemRow, { borderColor: colors.border }]}> 
              <View style={styles.itemInfo}>
                <Text style={[styles.itemName, { color: colors.foreground }]}>{item.name}</Text>
                <Text style={[styles.itemMeta, { color: colors.mutedForeground }]}>{item.category} · {formatCurrency(item.price)}</Text>
              </View>
              <View style={styles.qtyBox}>
                <Pressable onPress={() => updateQty(item.uid, item.qty - 1)} style={[styles.qtyBtn, { backgroundColor: colors.muted }]}><Feather name="minus" size={14} color={colors.foreground} /></Pressable>
                <Text style={[styles.qtyText, { color: colors.foreground }]}>{item.qty}</Text>
                <Pressable onPress={() => updateQty(item.uid, item.qty + 1)} style={[styles.qtyBtn, { backgroundColor: colors.muted }]}><Feather name="plus" size={14} color={colors.foreground} /></Pressable>
              </View>
              <Text style={[styles.itemTotal, { color: colors.primary }]}>{formatCurrency(item.price * item.qty)}</Text>
              <Pressable onPress={() => removeItem(item.uid)} hitSlop={10}><Feather name="x" size={18} color={colors.destructive} /></Pressable>
            </View>
          ))}
        </View>
        ...
        <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          ...
          <View style={styles.threeCol}>
            <NumberField label="GST %" value={draft.gstRate} onChange={(gstRate) => updateDraft({ gstRate })} />
            <NumberField label="Disc %" value={draft.discountRate} onChange={(discountRate) => updateDraft({ discountRate })} />
            <NumberField label="Delivery" value={draft.delivery} onChange={(delivery) => updateDraft({ delivery })} />
          </View>
        ...
        <View style={[styles.previewCard, { backgroundColor: colors.primary }]}>
          <View style={styles.previewHeader}>
            <View>
              <Text style={styles.previewStore}>{settings.name}</Text>
              <Text style={styles.previewMuted}>{settings.phone}</Text>
            </View>
            <View style={styles.previewRight}>
              <Text style={styles.previewMuted}>{draft.date}</Text>
              <Text style={styles.previewInvoice}>{draft.invoiceNum}</Text>
            </View>
          </View>
          <View style={styles.previewLine} />
          <Text style={styles.previewMuted}>Bill To</Text>
          <Text style={styles.previewCustomer}>{draft.customerName || "Customer not selected"}</Text>
          <View style={styles.totalRows}>
            ...
            <Row label="Delivery" value={formatCurrency(totals.delivery)} />
          </View>
          <View style={[styles.grandRow, { backgroundColor: "rgba(255,255,255,0.14)" }]}>
            <Text style={styles.grandLabel}>Total Amount</Text>
            <Text style={[styles.grandValue, { color: colors.accent }]}>{formatCurrency(totals.grand)}</Text>
          </View>
        </View>

        <View style={styles.actions}>
          <Pressable testID="save-bill" onPress={handleSave} style={[styles.actionButton, { backgroundColor: colors.primary }]}>
            <Feather name="save" size={19} color={colors.primaryForeground} />
            <Text style={[styles.actionText, { color: colors.primaryForeground }]}>Save bill and customer</Text>
          </Pressable>
          <Pressable onPress={clearDraft} style={[styles.actionButton, { backgroundColor: colors.secondary }]}>
            <Feather name="refresh-cw" size={18} color={colors.secondaryForeground} />
            <Text style={[styles.actionText, { color: colors.secondaryForeground }]}>New bill</Text>
          </Pressable>
        </View>
      </ScrollView>
      ...
      <Modal visible={productPickerOpen} animationType="slide" onRequestClose={() => setProductPickerOpen(false)}>
        <View style={[styles.modalScreen, { backgroundColor: colors.background, paddingTop: insets.top + (Platform.OS === "web" ? 50 : 8) }]}>
          <View style={styles.modalHeader}>
            <Text style={[styles.modalTitle, { color: colors.foreground }]}>Add products</Text>
            <Pressable onPress={() => setProductPickerOpen(false)}><Feather name="x" size={24} color={colors.foreground} /></Pressable>
          </View>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <Pressable onPress={() => { addItem(item); Haptics.selectionAsync(); }} style={[styles.productRow, { backgroundColor: colors.card, borderColor: colors.border }]}> 
                <View style={styles.itemInfo}>
                  <Text style={[styles.itemName, { color: colors.foreground }]}>{item.name}</Text>
                  <Text style={[styles.itemMeta, { color: colors.mutedForeground }]}>{item.category}</Text>
                </View>
                <Text style={[styles.itemTotal, { color: colors.primary }]}>{formatCurrency(item.price)}</Text>
      ...
      <Modal visible={customerPickerOpen} animationType="slide" onRequestClose={() => setCustomerPickerOpen(false)}>
        <View style={[styles.modalScreen, { backgroundColor: colors.background, paddingTop: insets.top + (Platform.OS === "web" ? 50 : 8) }]}>
          <View style={styles.modalHeader}>
            <Text style={[styles.modalTitle, { color: colors.foreground }]}>Saved customers</Text>
            <Pressable onPress={() => setCustomerPickerOpen(false)}><Feather name="x" size={24} color={colors.foreground} /></Pressable>
          </View>
          <FlatList
            data={customers}
            keyExtractor={(item) => item.id}
            contentContainerStyle={styles.modalList}
            renderItem={({ item }) => (
              <Pressable onPress={() => { loadCustomerIntoDraft(item); setCustomerPickerOpen(false); }} style={[styles.productRow, { backgroundColor: colors.card, borderColor: colors.border }]}>
                <View style={styles.itemInfo}>
                  <Text style={[styles.itemName, { color: colors.foreground }]}>{item.name}</Text>
                  <Text style={[styles.itemMeta, { color: colors.mutedForeground }]}>{item.phone || item.city || "No contact saved"}</Text>
                </View>
                <Text style={[styles.itemTotal, { color: colors.primary }]}>{formatCurrency(item.totalSpent)}</Text>
              </Pressable>
            )}
            ListEmptyComponent={<Text style={[styles.emptyText, { color: colors.mutedForeground, textAlign: "center", marginTop: 40 }]}>No saved customers yet.</Text>}
          />
        </View>
      </Modal>
    </View>
...
const styles = StyleSheet.create({
  ...
  hero: { borderRadius: 28, padding: 22, minHeight: 150, flexDirection: "row", justifyContent: "space-between", overflow: "hidden" },
  heroKicker: { color: "rgba(255,255,255,0.72)", fontSize: 13, fontFamily: "Inter_600SemiBold", textTransform: "uppercase", letterSpacing: 1.2 },
  heroTitle: { color: "#FFFFFF", fontSize: 34, fontFamily: "Inter_700Bold", marginTop: 6 },
  heroText: { color: "rgba(255,255,255,0.78)", fontSize: 14, marginTop: 8, fontFamily: "Inter_500Medium" },
  ...
  productRow: { flexDirection: "row", alignItems: "center", borderWidth: 1, borderRadius: 20, padding: 14, marginBottom: 10, gap: 12 },
  itemInfo: { flex: 1 },
  ...
  actionText: { fontSize: 15, fontFamily: "Inter_700Bold" },
